...
cursor.execute("""
WITH Rezerwacie_mies AS (
    SELECT 
        DATE_FORMAT(Data_rezerwacji, '%Y-%m') AS month,
        COUNT(*) AS ilosc_rezerwacji
    FROM Rezerwacje
    GROUP BY DATE_FORMAT(Data_rezerwacji, '%Y-%m')
),
wzrosty AS (
    SELECT 
        month,
        ilosc_rezerwacji,
        LAG(ilosc_rezerwacji) OVER (ORDER BY month) AS poprzedni,
        CASE 
            WHEN ilosc_rezerwacji > LAG(ilosc_rezerwacji) OVER (ORDER BY month) THEN 1 
            ELSE 0 
        END AS is_growth
    FROM Rezerwacie_mies
),
Streaks AS (
    SELECT 
        month,
        ilosc_rezerwacji,
        SUM(CASE WHEN is_growth = 0 THEN 1 ELSE 0 END) OVER (ORDER BY month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS streak_id
    FROM wzrosty
)
SELECT 
    MIN(month) AS start_month,
    MAX(month) AS end_month,
    COUNT(*) AS dlugosc_streak
FROM Streaks
GROUP BY streak_id
ORDER BY dlugosc_streak DESC
LIMIT 1;
""")

results = cursor.fetchall()
for row in results:
    poczatek_streak, koniec_streak, dlugosc_streak = row
...
wiek_grupy = {row[0]: row[1] for row in results}

sorted_wiek_grupy = sorted(wiek_grupy.items(), key=lambda x: x[1], reverse=True)

# Dwie najliczniejsze grupy
najliczniejsza_grupa1 = sorted_wiek_grupy[0]
najliczniejsza_grupa2 = sorted_wiek_grupy[1]

# Dwie najmniej liczne grupy
najmniej_liczna_grupa1 = sorted_wiek_grupy[-1]
najmniej_liczna_grupa2 = sorted_wiek_grupy[-2]

query_srednia = """
SELECT YEAR(CURDATE()) - YEAR(Data_urodzenia) AS Wiek
FROM Klienci
JOIN Rezerwacje ON Klienci.id_klienci = Rezerwacje.id_klienci
"""
cursor.execute(query_srednia)
wiek_values = [row[0] for row in cursor.fetchall()]

# Obliczenie średniego wieku
srednia_wiek = np.mean(wiek_values)
...
wycieczki_oceny = {row[0]: row[1] for row in results}
sorted_wycieczki_oceny = sorted(wycieczki_oceny.items(), key=lambda x: x[1], reverse=True)
najlepsza_wycieczka = sorted_wycieczki_oceny[0]
najgorsza_wycieczka = sorted_wycieczki_oceny[-1]
...
zysk_po_podatku = 0.81*suma_zysk_rok
...
\section*{{Wstęp}}
Wombat Grylls sp. z o.o. to biuro podróży specjalizujące się w organizacji wycieczek do Australii. Działamy wyłącznie online, co pozwala nam ograniczyć koszty operacyjne, takie jak wynajem lokalu, i w pełni skoncentrować się na zapewnianiu najwyższej jakości usług naszym klientom. Nasza firma rozpoczęła działalność 1 stycznia 2022 roku, a proces rekrutacji zakończył się 14 stycznia tego samego roku. Starannie dobrany zespół pracowników pozostaje z nami od początku istnienia firmy, a jego skład od tamtego czasu się nie zmienił.
Dzięki zdalnemu modelowi pracy nasi pracownicy mogą wykonywać swoje obowiązki z dowolnego miejsca na świecie. Zespół Wombat Grylls sp. z o.o. tworzą wykwalifikowani specjaliści, którzy są z nami od początku działalności firmy. W jego skład wchodzą: programista, menedżer, księgowy, specjalista ds. marketingu oraz trzech konsultantów ds. obsługi klienta. Nasi pracownicy są zatrudnieni na podstawie umów o pracę, co zapewnia im stabilne warunki zatrudnienia oraz pełne świadczenia wynikające z polskiego prawa pracy. Nasza firma wyróżnia się także podejściem do organizacji wycieczek. Każdy wyjazd jest indywidualnie przygotowywany, dlatego nie narzucamy maksymalnej liczby uczestników – dostosowujemy wszystko do aktualnych potrzeb klientów. Wycieczki organizowane przez Wombat Grylls sp. z o.o. trwają zazwyczaj od dwóch do trzech tygodni, co pozwala uczestnikom w pełni doświadczyć niezwykłości Australii, odwiedzając jej najpiękniejsze miejsca i biorąc udział w lokalnych atrakcjach. Wyżywienie jest zawsze wliczone w cenę noclegu, co pozwala na prostotę organizacyjną i eliminuje konieczność dodatkowych opłat. Z kolei wszelkie rezerwacje traktujemy jako wiążące – nie przyjmujemy zwrotów, ale dbamy o to, by klienci mieli pewność najwyższej jakości usług już od momentu dokonania rezerwacji. Dodatkowym ułatwieniem dla podróżnych jest fakt, że do wyjazdów do Australii nie są wymagane płatne wizy. Turyści mogą korzystać z bezpłatnych wiz turystycznych, które pozwalają na pobyt w Australii do trzech miesięcy, co znacząco skraca czas przygotowań i eliminuje dodatkowe formalności.
Każda rezerwacja jest rozdzielana na liczbę uczestników, co pozwala nam szczegółowo analizować zainteresowanie ofertą i lepiej dostosowywać przyszłe wyjazdy. W przypadku rezerwacji przykładamy szczególną wagę do tego, by klient w momencie jej składania był pełnoletni, co zapewniamy poprzez generowanie dat wyjazdów oraz rezerwacji z uwzględnieniem wieku uczestnika. Wycieczki można rezerwować z wyprzedzeniem, dlatego data rozpoczęcia wyjazdu i moment jego zarezerwowania są starannie zsynchronizowane. Dodatkowo, każdy klient w momencie rezerwacji musi podać dane bliskiej osoby, co jest wymaganiem w celu zapewnienia bezpieczeństwa i ułatwienia kontaktu w nagłych sytuacjach.
W trakcie wycieczek klienci mają możliwość pozostania w hotelu, jeśli nie mają siły uczestniczyć w zaplanowanych atrakcjach. Niemniej, koszty tych atrakcji nie podlegają zwrotowi, gdyż są one uwzględnione w pakiecie wyjazdu. Koszty organizacyjne wyjazdów są szczegółowo rozliczane, z uwzględnieniem takich elementów jak noclegi, loty, atrakcje, transport, przewodnicy czy ubezpieczenie. Wszystkie te elementy są starannie kalkulowane, aby zapewnić klientom jak najlepszy stosunek jakości do ceny.
Nasze wyjazdy cieszą się stałym zainteresowaniem, co świadczy o stabilnej pozycji firmy na rynku. Każdy rodzaj wycieczki obejmuje określone atrakcje, które są starannie zaplanowane i niezmienne – różnice dotyczą jedynie liczby dni spędzanych w Australii. Nie oferujemy dodatkowych atrakcji poza tymi przewidzianymi w standardowym programie, co pozwala na pełną przejrzystość oferty i zapewnia wszystkim uczestnikom tak samo wyjątkowe doświadczenia.
...
\\
    ...
    max_ilosc_rodzaj = max(Ilosc_osob, key = lambda x: x[1])
    min_ilosc_rodzaj = min(Ilosc_osob, key = lambda x: x[1])
...
\end{{center}}
Wycieczka o największej ilości rezerwacji:
\begin{{itemize}}
    \item \textbf{{id rodzaju:}} {max_ilosc_rodzaj[0]}
    \item \textbf{{Ilość rezerwacji:}} {max_ilosc_rodzaj[1]}
\end{{itemize}}
Wycieczka o najmniejszej ilości rezerwacji:
\begin{{itemize}}
    \item \textbf{{id rodzaju:}} {min_ilosc_rodzaj[0]}
    \item \textbf{{Ilość rezerwacji:}} {min_ilosc_rodzaj[1]}
\end{{itemize}}                      
...
\textbf{{Odpowiedź: }}Wycieczki są opłacalne, ale ich zyski zależą od wielu czynników, takich jak cena oraz czas trwania.
...
\end{{itemize}} 
Najdłuższy okres wzrostu:
\begin{{itemize}}
    \item \textbf{{Pierwszy miesiąc: }} {poczatek_streak}
    \item \textbf{{Ostatni miesiąc: }} {koniec_streak}
    \item \textbf{{Długość okresu: }} {dlugosc_streak}
\end{{itemize}} 
...
\textbf{{Odpowiedź: }}Firma w różnych okresach radzi sobie różnie – raz lepiej, raz gorzej, ale ostatecznie zyski pozwalają na dalsze prowadzenie działalności.
...
\section*{{Pytanie 4}}
Sprawdź, który z pracowników generuje największy zysk na przestrzeni kolejnych lat. Należy uzwględnić całkowity przychód, koszty organizacij wycieczki oraz koszty pracownika, na które składa się jego miesięczna pensja oraz koszt stanowiska.
...
\end{{center}}
\subsubsection*{{Podsumowanie efektywności pracowników}}"""))
    
    for rok in sorted(data_by_year.keys()):
        if rok == 2025:
            continue
        dane_roku = data_by_year[rok]
        max_zysk = max(dane_roku, key=lambda x: x['zysk_netto'])
        doc.append(NoEscape(rf"""
Rok {rok}:
\begin{{itemize}}
    \item \textbf{{Najbardziej efektywny pracownik:}} ID {max_zysk['id_pracownicy']}
    \item \textbf{{Zysk netto:}} {max_zysk['zysk_netto']:,.2f} PLN
\end{{itemize}}
"""))
    doc.append(NoEscape(rf"""
                        
\section*{{Pytanie 5}}
Prezentacja i omówienie wyników finansowych firmy od początku działalności.
...
\begin{{itemize}}
    ...
    \item \textbf{{Podatek CIT: }} 19\%
    \item \textbf{{Zysk całkowity: }} {zysk_po_podatku} PLN
\end{{itemize}}

\section*{{Pytanie 6}}
Jakie są najbardziej i najmniej liczne grupy wiekowe klientów w firmie? Jaki
jest średni wiek klientów w tej firmie?

\begin{{center}}
\includegraphics[width = \textwidth]{{Demografia_wiekowa.png}}
\end{{center}}

Najbardziej liczne grupy wiekowe:
\begin{{itemize}}
    \item \textbf{{Przedział wiekowy:}} {najliczniejsza_grupa1[0]} \textbf{{z liczbą klientów:}} {najliczniejsza_grupa1[1]}
\end{{itemize}}
\begin{{itemize}}
    \item \textbf{{Przedział wiekowy:}} {najliczniejsza_grupa2[0]} \textbf{{z liczbą klientów:}} {najliczniejsza_grupa2[1]}
\end{{itemize}}

Najmniej liczne grupy wiekowe:
\begin{{itemize}}
    \item \textbf{{Przedział wiekowy:}} {najmniej_liczna_grupa1[0]} \textbf{{z liczbą klientów:}} {najmniej_liczna_grupa1[1]}
\end{{itemize}}
\begin{{itemize}}
    \item \textbf{{Przedział wiekowy:}} {najmniej_liczna_grupa2[0]} \textbf{{z liczbą klientów:}} {najmniej_liczna_grupa2[1]}
\end{{itemize}}

Średnia wieku klientów:
\begin{{itemize}}
    \item \textbf{{Średni wiek:}} {srednia_wiek:.2f} lat
\end{{itemize}}

\section*{{Pytanie 7}}
Jakie są najlepiej i najgorzej oceniane wycieczki w firmie, a także ich średnie
oceny?
...
Najlepiej oceniana wycieczka:
\begin{{itemize}}
    \item \textbf{{Nazwa:}} {najlepsza_wycieczka[0]} 
    \item \textbf{{Średnia ocena:}} {najlepsza_wycieczka[1]:.2f}
\end{{itemize}}

Najgorzej oceniana wycieczka:
\begin{{itemize}}
    \item \textbf{{Nazwa:}} {najgorsza_wycieczka[0]}
    \item \textbf{{Średnia ocena:}} {najgorsza_wycieczka[1]:.2f}
\end{{itemize}}
